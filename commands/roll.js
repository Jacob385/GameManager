...
module.exports = {
  ...
  async execute (interaction) {
    ...
    const die = interaction.options.getInteger('d') ?? 20
    let quantity = interaction.options.getInteger('quantity') ?? 1
    const advantage = interaction.options.getInteger('advantage') ?? null
    const modifier = interaction.options.getInteger('modifier') ?? 0
    const advantageMagnitude = interaction.options.getInteger('magnitude') ?? 1
    const extra = interaction.options.getInteger('extra') ?? null

    const set = []
    let sum = 0
    let list = '('

    let advantageString// used to print out witch 'advantage' is used
    switch (advantage) {
    ...
    let bumpPoint// used to bump the roll up to a least this value
    let extraString// used to print out witch 'extra' is used
    switch (extra) {
    ...
    for (let y = 0; y < quantity; y++) {
      let winningEmphasisMatch = false
