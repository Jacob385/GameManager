...
module.exports = {
  ...
  async execute (interaction) {
    ...
    const interactionID = interaction.id// TODO remove if not used
      ...
      checkWin = (x, y, game) => {
        ...
        for (let direction = 0; direction < vectorArray.length; direction++) {
          ...
          for (let step = 0; step < 3; step++) {
            ...
            try {
              if (game.board[currentCell.x][currentCell.y] === color) { inARow[direction]++ } else { step = 3 }
            } catch (error) { step = 3 }
          }
        }
          ...
          for (let x = 0; x < gameHolder.length; x++) {
            ...
            if (i.message.id === gameHolder[x].gameID) {
              ...
              if (gameHolder[x].player1 === i.user.id || gameHolder[x].player2 === i.user.id) {
                ...
                // if a user hits quit
                if (i.customId === 'column-1' && (i.user.id === gameHolder[x].player1 || i.user.id === gameHolder[x].player2)) { return true }

                const buttonList = ['column1', 'column2', 'column3', 'column4', 'column5', 'column6', 'column7']
